...
class Client(object):
    ...
    def __init__(self, rootwrap_daemon_cmd):
        ...
        self._manager = None
        self._proxy = None
        log_utils.log_message("executed: self._proxy = None")
        self._process = None
        self._finalize = None
    ...
    def _initialize(self):
        ...
        log_utils.log_message("executed: self._proxy = self._manager.rootwrap()")
    ...
    def _run_one_command(self, proxy, cmd, stdin):
        LOG.info('%s() caller: %s()', log_utils.get_fname(1), log_utils.get_fname(2))
        LOG.info('%s(): command: [%s]', sys._getframe(0).f_code.co_name, cmd)
        ...
        try:
            _need_restart = True
            LOG.info('%s(): started', log_utils.get_fname(1))
            res = proxy.run_one_command(cmd, stdin)
            LOG.info('%s(): complete', log_utils.get_fname(1))
            _need_restart = False
            return res
        finally:
            if _need_restart:
    ...
    def execute(self, cmd, stdin=None):
        ...
        with self._exec_sem:
            ...
            proxy = self._proxy
            log_utils.log_message("executed: proxy = self._proxy")
            retry = False
            if self._need_restart:
                proxy = self._restart(proxy)
                log_utils.log_message("executed: proxy = self._restart(proxy)")
            try:
